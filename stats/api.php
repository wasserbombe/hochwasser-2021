<?php
    ...
    function cachedDBQuery($sql){
        ...
        if (!isset($_REQUEST["forceLive"]) && file_exists($cache_fn) && filemtime($cache_fn) > time()-60*60){
    ...
    if (isset($_REQUEST["view"]) && !empty($_REQUEST["view"])){
        if ($_REQUEST["view"] == "tweets-per-day"){
            ...
            $res["data"] = $graphdata;
            $res["code"] = 200; 
        } elseif ($_REQUEST["view"] == "all-topics"){
            $data = cachedDBQuery("SELECT *
                                    FROM (
                                        SELECT DISTINCT IFNULL(th.`topic`, LOWER(ht.`hashtag`)) AS 'topic', COUNT(*) AS 'tweets'
                                        FROM hashtag2tweet ht
                                        LEFT JOIN topics_hashtags th ON LOWER(ht.`hashtag`) = th.`hashtag`
                                        GROUP BY IFNULL(th.`topic`, LOWER(ht.`hashtag`))
                                        ORDER BY IFNULL(th.`topic`, LOWER(ht.`hashtag`))
                                    ) t1
                                    WHERE t1.tweets > 25");
            $res["data"] = $data; 
            $res["code"] = 200;
